...
@if(Auth::user()->role != 'user')
    <a href="{{ URL::route('backend.tds.index') }}">All ({{ $count_all }})</a>

    @if($count_approve)
    | <a href="{{ URL::route('backend.tds.index', ['status' => 'approve']) }}">Approved ({{ $count_approve }})</a>
    @endif

    @if($count_draft)
    | <a href="{{ URL::route('backend.tds.index', ['status' => 'draft']) }}">Drafted ({{ $count_draft }})</a>
    @endif

    @if($count_pending)
    | <a href="{{ URL::route('backend.tds.index', ['status' => 'pending']) }}">Pending ({{ $count_pending }})</a>
    @endif

    @if($count_trash)
    | <a href="{{ URL::route('backend.tds.index', ['status' => 'trash']) }}">Trashed ({{ $count_trash }})</a>
    @endif
@endif
    ...
    <thead>
        <tr>
            ...
            <th>
                ...
                <a href="{{ URL::route('backend.tds.add', ['v' => 2]) }}" class="btn btn-primary btn-sm pull-right"><i class="fa fa-plus"></i> Add New</a>  
                
                @if(Input::get('status') == 'trash')
                <a class="btn btn-default btn-sm pull-right delete"
                data-href="{{ URL::route('backend.tds.empty-trash') }}" 
                data-toggle="modal"
            ...
            <td>
                <div class="task-config-btn btn-group pull-right">
                    ...
                    <ul class="dropdown-menu pull-right">
                        @if(Auth::user()->role == 'admin')
                        <li><a href="{{ URL::route('backend.tds.edit', [$row->lead_id, 'v' => 2]) }}">Edit</a></li>
                        @endif
                        <li><a href="{{ URL::route('backend.tds.pdf-view', [$row->lead_id, $row->token, 'v' => $version]) }}" download>Download PDF</a></li>
                        <li><a href="{{ URL::route('backend.tds.pdf-view', [$row->lead_id, $row->token, 'v' => $version]) }}" target="_blank">View PDF</a></li>
                        <li>
                            <a href="#" class="embed" 
                                data-href="" 
                                data-toggle="modal"
                                data-target=".embed-modal"
                                data-embed="{{ URL::route('backend.tds.pdf-view', [$row->lead_id, $row->token, 'v' => $version]) }}">Embed</a>                    
                        </li>
                        @if(Auth::user()->role == 'admin')
                        <li>
                            <a href="#" class="delete"
                                data-href="{{ URL::route('backend.tds.duplicate', $row->lead_id) }}" 
                                data-toggle="modal"
                                data-target=".delete-modal" 
                        ...
                        <li>
                            <a href="#" class="delete"
                                data-href="{{ URL::route('backend.tds.destroy', $row->lead_id) }}" 
                                data-toggle="modal"
                                data-target=".delete-modal" 
